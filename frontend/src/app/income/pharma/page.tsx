...
import { Upload, X, Eye, Banknote, CreditCard, Wallet, List, LayoutDashboard, FileText, Search, CheckCircle, AlertTriangle } from "lucide-react";
...
export default function IncomePharmaPage() {
  ...
  const fetchTxns = async (p = page, s = search, fd = fromDate, td = toDate, pm = txnPaymentFilter, ps = txnPymtStatusFilter, ts = txnStatusFilter) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    if (activeTab === "transactions" && hasSearched) fetchTxns();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeTab, dashFromDate, dashToDate, logPage]);

  const handleDashFilter = () => fetchDashboard(dashFromDate, dashToDate);
  ...
  const handleRowClick = (txn: PharmaTxn) => {
    sessionStorage.setItem("pharmaFilterState", JSON.stringify({
      activeTab, page, search, fromDate, toDate, txnPaymentFilter, txnPymtStatusFilter, txnStatusFilter,
    }));
    ...
    if (saved) {
      try {
        ...
        if (s.activeTab === "transactions") {
          ...
          setHasSearched(true);
          fetchTxns(s.page || 1, s.search || "", s.fromDate || "", s.toDate || "", s.txnPaymentFilter || "", s.txnPymtStatusFilter || "", s.txnStatusFilter || "");
        }
      } catch {
        /* ignore */
      }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const { data } = await api.post("/income/pharma/import", formData, { headers: { "Content-Type": "multipart/form-data" } });
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "REVIEW_REQ": return "bg-amber-50 text-amber-600";
  ...
  return (
    <DashboardLayout>
      <div>
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-400">Manage pharmacy sales, payments, and imports</p>
          </div>
        </div>
        ...
        <div className="bg-slate-100 rounded-xl p-1 max-w-2xl mb-6">
          <div className="flex">
            {[
              ...
              { key: "importlog" as const, label: "Pharma Data Import", icon: FileText },
            ].map((tab) => (
              <button
                key={tab.key}
        ...
        {activeTab === "dashboard" && (
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <button onClick={() => handleCardClick("CASH")} className="bg-white rounded-2xl border border-slate-200/60 p-5 hover:border-emerald-300 hover:shadow-md transition-all text-left">
              ...
              <button onClick={() => handleCardClick("BANK")} className="bg-white rounded-2xl border border-slate-200/60 p-5 hover:border-blue-300 hover:shadow-md transition-all text-left">
                <div className="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center mb-3">
                  <Wallet size={20} className="text-blue-500" />
                </div>
                <p className="text-sm text-slate-400">Bank / UPI / Card</p>
                <p className="text-xl font-bold text-slate-800">{formatCurrency(dashboard.bank)}</p>
                <p className="text-xs text-slate-400 mt-1">Click to view details</p>
              </button>
              <button onClick={() => handleCardClick("CREDIT")} className="bg-white rounded-2xl border border-slate-200/60 p-5 hover:border-amber-300 hover:shadow-md transition-all text-left">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 p-4 mb-6">
              <div className="flex flex-wrap items-center gap-3">
                <div className="relative flex-1 min-w-[200px] max-w-xs">
                  ...
                  <input type="text" placeholder="Bill No, Patient, UHID, IP No..." value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSearch()} className="w-full pl-9 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" />
                </div>
                ...
                <select value={txnStatusFilter} onChange={(e) => setTxnStatusFilter(e.target.value)} className="px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500">
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {!loading && !hasSearched && (
                      <tr><td colSpan={9} className="text-center text-slate-400 py-12">Use the search or date filter to view transactions</td></tr>
                    )}
                    {loading && (
                    ...
                    {!loading && hasSearched && txns.length === 0 && (
                      <tr><td colSpan={9} className="text-center text-slate-400 py-12">No transactions found</td></tr>
                    )}
                    {!loading && txns.map((txn) => {
                      const isReviewReq = txn.txn_status === "REVIEW_REQ";
                      return (
                        <tr key={txn.id} onClick={() => handleRowClick(txn)} className={`border-b border-slate-100 last:border-0 cursor-pointer group hover:bg-slate-100/80 transition-colors ${txn.txn_status === "ERROR" ? "bg-red-50/50" : ""}`}>
                          ...
                          <td className={`px-5 py-3.5 font-medium group-hover:text-purple-700 ${isReviewReq ? "text-amber-600" : "text-slate-700"}`}>{txn.billNo}</td>
        ...
        {activeTab === "importlog" && (
          <>
            <div className="flex items-center justify-between mb-4">
              ...
              <Button size="sm" onClick={() => { setImportResult(null); setSelectedFile(null); setImportModalOpen(true); }}>
                <Upload size={16} className="mr-1" /> Import Pharma Billing
              </Button>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200/60">
                      ...
                      <th className="text-center px-5 py-3.5 font-semibold text-slate-500 text-xs uppercase tracking-wider text-emerald-500">Inserted</th>
                      <th className="text-center px-5 py-3.5 font-semibold text-slate-500 text-xs uppercase tracking-wider hidden sm:table-cell text-amber-500">Updated</th>
                      <th className="text-center px-5 py-3.5 font-semibold text-slate-500 text-xs uppercase tracking-wider hidden sm:table-cell">Skipped</th>
                      <th className="text-center px-5 py-3.5 font-semibold text-slate-500 text-xs uppercase tracking-wider text-red-500">Failed</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    ...
                    {!logLoading && importLogs.length === 0 && (
                      <tr><td colSpan={9} className="text-center py-12 text-slate-400">No imports yet</td></tr>
                    )}
                    {!logLoading && importLogs.map((log) => (
                      <tr key={log.id} className="hover:bg-slate-50/50">
                        <td className="px-5 py-3.5 font-medium text-slate-700">{log.fileName}</td>
                        <td className="px-5 py-3.5 hidden sm:table-cell">
                          <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-50 text-indigo-600">Pharma Billing</span>
                        </td>
                        <td className="px-5 py-3.5 text-center text-slate-600">{log.totalRecords}</td>
                        <td className="px-5 py-3.5 text-center text-emerald-600">{log.inserted}</td>
                        <td className="px-5 py-3.5 text-center hidden sm:table-cell text-amber-600">{log.updated}</td>
                        <td className="px-5 py-3.5 text-center hidden sm:table-cell text-slate-500">{log.skipped}</td>
                        <td className="px-5 py-3.5 text-center text-red-600">{log.failed}</td>
                        <td className="px-5 py-3.5 hidden lg:table-cell text-slate-500">{new Date(log.importStarted).toLocaleString("en-GB")}</td>
                        <td className="px-5 py-3.5">
                          {log.failed > 0 && (
                            <button onClick={() => handleViewErrors(log)} className="inline-flex items-center gap-1 text-xs font-medium text-red-600 hover:bg-red-50 px-2 py-1 rounded-lg transition-colors">
      ...
      <Modal isOpen={importModalOpen} onClose={() => { if (!importing) setImportModalOpen(false); }} title="Import Pharma Billing">
        <div className="space-y-4">
          <p className="text-sm text-slate-500">
            Upload a Pharma Sales Excel file with columns: <strong>S.No, Entry Name, Entry Date, Entry No, Customer, Total Amt, Discount, Tax, Net Amount, Patient_name, Payment Mode, Mobile No, Credit Status</strong>
          </p>
          <p className="text-xs text-slate-400">
            Patients are matched when Customer starts with GCCH (UHID before the dash). Payment Mode may be a CSV (e.g. Cash,Bank). Credit sales create receivables; multi-mode or Bank payments are flagged for review.
          </p>
